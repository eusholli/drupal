...
// Version 1.0 @Lars Hendahl @Mikael Ek
//------------------------------------------------------------------------------------------------------------------
...
// sIFR fonts
var eSifrEricsson = {
		src: '/shared/eipa/swf/eSifrEricsson.swf',
...
var eSifrEricssonMaster = {
	src: '/shared/eipa/swf/eSifrEricssonMaster.swf',
	ratios:[6,1.24,7,1.21,8,1.18,10,1.16,11,1.13,12,1.12,13,1.11,15,1.1,17,1.09,19,1.08,22,1.07,26,1.06,32,1.05,41,1.04,58,1.03,97,1.02,1.01],
	wmode: 'transparent'
};

var eSifrEricssonBlueScarlet = {
	src: '/shared/eipa/swf/eSifrEricssonBlueScarlet.swf',
	ratios: [6,1.24,7,1.21,8,1.18,10,1.16,11,1.13,12,1.12,13,1.11,15,1.1,17,1.09,19,1.08,22,1.07,26,1.06,32,1.05,41,1.04,58,1.03,97,1.02,1.01],
	wmode: 'transparent'
};
sIFR.activate(eSifrEricsson,eSifrEricssonMaster,eSifrEricssonBlueScarlet); 
sIFR.replace(eSifrEricssonMaster, {selector: 'h1.eH1Sifr'});
...
YAHOO.namespace("Ericsson.validate");
YAHOO.Ericsson.validate = {
	init: function() {				
		try {
		...
		for (i=0, n=this.targetForm.length; i<n; i++) {
			YUE.addListener(this.targetForm[i], "submit", this.getRequiredElem, this);
		}
	},
	getRequiredElem: function(e, obj) {
		this.elem = YU.Dom.getElementsByClassName("eRequired", "*", this);		
		for (i=0, n=this.elem.length; i<n; i++) {
			YAHOO.Ericsson.validate.validateElem(this.elem[i]);
		}
		if (!YAHOO.Ericsson.validate.finalExam(this)) {			
			YUE.preventDefault(e);
		}
	},
	validateElem: function(obj) {
		obj.value.length > 0 ? YU.Dom.replaceClass(obj, "eFail", "eSuccess") : YU.Dom.replaceClass(obj, "eSuccess", "eFail");		
	},
	finalExam: function(obj) {
		return YU.Dom.getElementsByClassName("eFail", "*", obj).length === 0;		
	}
};
 ...
 * 
 * @TODO
 * Add Animation to career active state
 * 
 * @NOTE
 * - Instantiate explicitly
 * - Populate with JSON or page mark-up
 */
...
YAHOO.Ericsson.Carousel = function(target, cfg, json) {
	...
	if(json && json.length > 0) {
		...
		for(i = 0, n = json.length; i < n; i++) {
			...
			linkText = document.createTextNode("Learn more about " + json[i].first_name);
...
YAHOO.Ericsson.selects = {
	init: function () {
		...
		for (i = 0, n = targets.length; i < n; i = i + 1) {					
			...
			// Skip Country select
			if (
				containerId != 'eCountryMenuContainer' && 
...
YAHOO.namespace.prefill = function () {	
...
var eNCMenu = function(){
	...
	var oeNCSelectedMenuItemChange = function (event) {
		...
		eOpenChildPage(oMenuItem.value);
	};	
...
//Open child page
var eOpenChildPage = function (eOpenChildPageUrl){
    this.location.href= eOpenChildPageUrl;
}

//FlowPlayer
var eFlowPlayerInit = function () {
	if (document.getElementById('eFlvStory1')) {			
		flowplayer("eFlvStory1", {
			src: "/shared/eipa/swf/flowplayer-3.1.3.swf", 
			wmode: "transparent"
		},
		{ 
			clip: { 
...
YAHOO.util.Event.onContentReady("eGlobalMenu", function () {
    ...
    /* Hot fix for bug #2402. 
     * This should be replaced by a proper CSS solution in future release! */
...
var eOnWindowLoadInit = function () {
	//YAHOO.Ericsson.validate.init();
	//YAHOO.Ericsson.buttons.init();
