...
// Created: 11/20/2009 11:51:16 AM

function WebTrends(){
	var that=this;
	// begin: user modifiable
	this.dcsid="dcsrftdicme5tdblqnprqdfcv_8r3j";
	this.domain="webtrends-lb.ericsson.net";
	//this.domain="webtrends-lb.sw.ericsson.se";
	this.timezone=1;
	this.fpcdom=".www.ericsson.com";
	this.onsitedoms="";
	this.downloadtypes="xls,doc,pdf,txt,csv,zip";
	this.navigationtag="div,table";
	this.customerfpc="Ericsson";
...
WebTrends.prototype.dcsOther=function(){
    var getCookie=this.dcsGetCookie;
    var custfpc=this.customerfpc;
    var fpc=this.fpc;
	if (getCookie(custfpc)){
		gTempWtId=getCookie(custfpc);
		this.dcsFPC();
	}
	else if (getCookie(fpc)){
		document.cookie=fpc+"=; expires=Thu, 01-Jan-1970 00:00:01 GMT";
	}
}
...
//Add attribute target to link with external link
var eExternalLinks =  function() {
	if (document.getElementsByTagName) {
		...
		for (var i = 0; i < anchorsLength; ++i) {
			...
			if (anchor.getAttribute("rel") == "external" || anchor.getAttribute("rel") == "external nofollow") {
				...
				YUE.addListener(anchor,"click", this.eAddStatsEvent, this);
			}			
			var eHref = anchor.getAttribute("href");
			var eTaleoLink = eHref.indexOf("https://tas-ericsson.taleo.net/");
			if (eTaleoLink != -1){
				anchor.target = "taleo";
				YUE.addListener(anchor,"click", this.eAddStatsEvent, this);
			}
		...
		var forms = YUD.getElementsByClassName("eTaleoForm", "form"),
		formsLength = forms.length,
		form;
		for (var i = 0; i < formsLength; ++i) {
			forms[i].target = "taleo";
		}
	}
}
var eAddStatsEvent = function (e,obj){
	var eLabel = this.innerHTML;
	var eUrl = this.getAttribute("href");
	var eFromPage = document.location.href;
	var ePageTitle = document.title;
	dcsMultiTrack("WT.cg_n",ePageTitle + " " + eFromPage,"WT.cg_s",eLabel + " " + eUrl);
}
//On Dom ready init
var eWbInit = function() {
	eExternalLinks();
}
YUE.onDOMReady(eWbInit); 

//Events
//globe.loaded 			null
//story.preview			1234
//story.overlay			1234
//story.video.play		1234
//story.video.end		1234

YAHOO.namespace("Ericsson.Webtrends");
YAHOO.Ericsson.Webtrends.registerEvent = function(event, id) {
	 dcsMultiTrack("WT.cg_n","FINAL Story:" + id,"WT.cg_s",event);
}
